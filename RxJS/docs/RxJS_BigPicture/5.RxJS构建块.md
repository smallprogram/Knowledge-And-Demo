...
Observable对象只能为当个Observer生成值，我们称之为单播。而RxJS的Subject可以为多个观察者产生相同的值，我们把者叫多播。在许多情况下，多播很有用。并且RxJS还有很多Operators，这些Operators可以让多播数据变得容易，并且不用设置和管理有关Subject的细节。
